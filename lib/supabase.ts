...
export async function deleteInvoice(id: string) {
  // Primeiro, buscar a invoice para obter o pdf_url se existir
  ...
  if (fetchError && fetchError.code !== 'PGRST116') {
    // PGRST116 = nenhuma linha encontrada, não é um erro crítico
    throw fetchError;
  }

  // Se houver PDF associado, deletar do storage
  if (invoice?.pdf_url) {
    try {
      await deleteInvoicePdf(invoice.pdf_url);
    } catch (error: any) {
      // Log do erro mas não bloquear a exclusão da invoice
      console.warn('Erro ao deletar PDF da invoice:', error);
    }
  }
...
export async function uploadInvoicePdf(
  ...
  // Obter o ID do usuário autenticado para definir como owner
