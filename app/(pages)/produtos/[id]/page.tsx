...
export default async function ProductPage({ params }: ProductPageProps) {
  ...
  if (isUUID) {
    // Tentar buscar no Supabase se o ID for um UUID válido
    try {
      product = await getSupabaseProductById(id)
      if (product) {
      ...
      if (isNotFoundError) {
        console.log(`Product with UUID ${id} not found in Supabase`)
        notFound()
        return
      }
    ...
    // Se não é UUID, tentar buscar nos produtos mock
    const numericId = parseInt(id, 10)
    if (!isNaN(numericId)) {
      product = getProductById(numericId)
    } else {
      // Se não é nem UUID nem número, produto não existe
      notFound()
      return
    }
  }
